  ...
  <div class="container-fluid">
    <div class="card card-primary">
      <div class="card-header">
        <h3 class="card-title">Form Tambah Program Anggaran</h3>
      </div>

      <form action="{{ route('program_anggaran.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
          <div class="form-group">
            <label for="name">Nama Program</label>
            <input type="text" name="name" id="name" class="form-control" placeholder="Masukkan nama program" value="{{ old('name') }}">
          ...
          <div class="form-group">
            ...
            <select name="jenis_program_anggaran_id" id="jenis_program_anggaran_id" class="form-control">
              ...
              @foreach($jenis as $j)
                <option value="{{ $j->id }}" {{ old('jenis_program_anggaran_id') == $j->id ? 'selected' : '' }}>{{ $j->jenis }}</option>
              @endforeach
            </select>
